...
class NetworkTrainer:
    ...
    def train(self, args):
        ...
        if train_text_encoder and args.stop_text_encoder_training is None:
            args.stop_text_encoder_training = args.max_train_steps + 1
        ...
        for epoch in range(num_train_epochs):
            ...
            if ip_noise_gamma > 0 and epoch >= ip_noise_start and epoch < ip_noise_end:
                if epoch == ip_noise_start:
                    accelerator.print("\nenabling IP noise")
                if epoch == ip_noise_end:
                    accelerator.print("\ndisabling IP noise")
                args.ip_noise_gamma = ip_noise_gamma * pow(args.ip_noise_factor, epoch)

            if v_pred_like_loss > 0 and epoch >= v_pred_like_loss_start and epoch < v_pred_like_loss_end:
                if epoch == v_pred_like_loss_start:
                    accelerator.print("\nenabling v pred like loss")
                if epoch == v_pred_like_loss_end:
                    accelerator.print("\ndisabling v pred like loss")
                args.v_pred_like_loss = v_pred_like_loss * pow(args.v_pred_like_loss_factor, epoch)

            if multires_noise_discount > 0 and epoch >= multires_noise_start and epoch < multires_noise_end:
                if epoch == multires_noise_start:
                    accelerator.print("\nenabling multires noise")
                if epoch == multires_noise_end:
                    accelerator.print("\ndisabling multires noise")
                args.multires_noise_discount = multires_noise_discount * pow(args.multires_noise_factor, epoch)
                args.multires_noise_iterations = int(multires_noise_iterations * pow(args.multires_noise_factor, epoch))
